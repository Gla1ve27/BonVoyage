...
categoryButtons.forEach((button) => {
  button.addEventListener("click", () => {
    // Remove active class and reset images for all buttons
    categoryButtons.forEach((btn) => {
      const img = btn.querySelector("img");
      if (img) {
        const originalSrc = img.src.replace("-active", "");
        img.src = originalSrc; // Reset image to original
      }
      btn.classList.remove("active");
    });

    // Add active class to the clicked button and update its image
    button.classList.add("active");
    const clickedImg = button.querySelector("img");
    if (clickedImg) {
      const srcWithoutExtension = clickedImg.src.split(".png")[0];
      clickedImg.src = `${srcWithoutExtension}-active.png`; // Add -active to the image source
    }
  });
});
